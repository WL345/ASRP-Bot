import discord.ext, config, toml

class BoloButtons(discord.ui.View):
    def __init__(self, log_msg, roblox_id, bot):
        super().__init__()
        self.log_msg = log_msg
        self.roblox_id = roblox_id
        self.bot = bot

    @discord.ui.button(label="Automated Ban-Bolo", style=discord.ButtonStyle.blurple, custom_id="ban-bolo:automated",)
    async def automated(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "This is not currently a function! Please use the manual button instead!", ephemeral=True)

    @discord.ui.button(label="Manual Ban-Bolo", style=discord.ButtonStyle.blurple, custom_id="ban-bolo:manual")
    async def manual(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            view = CompletionButton(self.log_msg, interaction.message, self.bot, self.roblox_id)
...
class CompletionButton(discord.ui.View):
    def __init__(self, log_msg, req_msg, bot, roblox_id):
        super().__init__()
        self.log_msg = log_msg
    ...
    async def completed(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if type(self.log_msg) == int:
                self.log_msg = await self.bot.get_channel(config.channels.ADMIN_REQUESTS).fetch_message(self.log_msg)
...
> - **Requested** {config.current_time()}""",
        ...
        view = BoloButtons(log_msg, roblox_id, self.bot)
        await self.bot.get_channel(config.channels.ADMIN_REQUESTS).send("<@&1182827674599706624>", embed=embed, view=view)
        await interaction.response.send_message("Successfully created a ban-bolo request.", ephemeral=True)

        try:
            data = toml.load("storage/misc.toml")
            data["interactions"]["ban-bolo"][roblox_id] = log_msg.id
            with open("storage/misc.toml", "w", encoding="utf-8") as f:
                toml.dump(data, f)
        except Exception as e:
            print(e)
