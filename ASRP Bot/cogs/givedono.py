...
class GiveDonoCog(discord.ext.commands.Cog):
    ...
    async def get_user_id(self, interaction: discord.Interaction, user: discord.User, donation: str):
        print(f"\n{interaction.user.name} - <@{interaction.user.id}> - Used the dono command")
        if donation == "silver":
            embed = discord.Embed(
...
> - **Time Given**: {config.current_time()}""",
    ...
    > - **Time Given**: {config.current_time()}""",
        ...
        await self.bot.get_channel(config.channels.SUPPORTERS).send(embed=embed)
        await self.bot.get_channel(config.channels.SUPPORTERS_LOGGING).send(embed=log_embed)
